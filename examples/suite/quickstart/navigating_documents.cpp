...
static void run_examples() {
    ...
    // it throws for a missing key. JsonOutOfRange derives from
    // JsonException (not std::out_of_range), so it must be caught by
    // its own type or by the JsonException base.
    ...
    for (const auto& entry : address.entries())
        std::cout << entry.first << ": " << address.find(entry.first)->asString() << "\n";
    std::cout << "\n";
