...
TEST(MaxDepth, array_nesting_at_max_depth_accepted) {
...
TEST(MaxDepth, array_nesting_beyond_max_depth_rejected) {
...
TEST(MaxDepth, object_nesting_at_max_depth_accepted) {
    std::string json;
...
TEST(MaxDepth, object_nesting_beyond_max_depth_rejected) {
    constexpr int depth = kMaxDepth + 1;
...
TEST(MaxDepth, mixed_alternating_nesting_bounded) {
    constexpr int depth = kMaxDepth + 1;
...
TEST(MaxDepth, many_siblings_at_shallow_depth_not_limited) {
    std::string json = "[";
    ...
    Json j = Json::parse(json);
    EXPECT_EQ(j.size(), 10000);
    EXPECT_EQ(j[9999].asNumber(), 9999.0);
}
