...
TEST(ParallelParsing, ProducesCorrectIndependentResults) {
    ...
    for (int t = 0; t < kThreads; ++t)
        EXPECT_EQ(results[t], static_cast<double>(t + t * 10)) << "thread " << t;
}

// Verifies many threads parsing the exact same input concurrently all
// produce equal results.
TEST(ParallelParsing, IdenticalInputIsDeterministic) {
...
TEST(ParallelParsing, ErrorsDoNotAffectOtherThreads) {
    constexpr int kThreads = 8;
    std::vector<int> ok(kThreads, 0);
    std::vector<std::thread> threads;
...
TEST(ParallelParsing, HighVolumePerThread) {
