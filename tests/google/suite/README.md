...
This document describes the test categories under `gtest/` — what each one
verifies, and the individual test files it contains.

| Category | Focus |
|---|---|
| [Concurrency](#concurrency) | Thread-safety across shared-read and independent instances |
| [Integration](#integration) | Multiple components working together end-to-end |
| [Lifecycle](#lifecycle) | Construction, destruction, copying, moving, exception safety |
| [Regression](#regression) | Previously identified parsing/storage edge cases that stay fixed |
...
Verifies thread-safety — JsonPro has no internal synchronization, so its
guarantee is the standard library one: concurrent const access to a shared
value is safe, and threads with no shared object need no synchronization at
all. These tests validate both edges of that guarantee, plus that
`Json::parse()` itself carries no shared state across threads.
...
full parse-modify-serialize round trip — rather than a single function in
isolation.
...
and moving — across the different value kinds a JSON value can hold (null,
bool, number, string, array, object), for both `Json` and `JsonObject`.
...
Verifies that a specific, previously identified parsing or storage edge case
stays correctly handled. One test file per class of edge case.
...
- **One suite name per file, matching its filename in PascalCase** — e.g.
  `object_storage.cpp` → `TEST(ObjectStorage, ...)`, `max_depth.cpp` →
  `TEST(MaxDepth, ...)`. Case names stay the same descriptive snake_case
  used throughout the rest of the codebase (e.g.
  `erase_middle_then_insert_appends_correctly`).
- **No manual registration.** `TEST(Suite, Case) { ... }` self-registers at
  static-init time — there's no `REGISTER_TEST_SUITE()` call, `run_tests()`
  dispatcher, or sequential id (`U1`, `L2`, etc.) to maintain; `ctest`/the
  test binary discovers every `TEST` automatically.
- **`EXPECT_*` is the default; `ASSERT_*` is the exception, not the rule.**
  Use `ASSERT_*` only where a failure would make continuing the test unsafe
  or meaningless — dereferencing a possibly-null pointer (`find()`'s
  result), or indexing into a container whose size was just checked.
  Everywhere else, prefer `EXPECT_*` so a single failing check doesn't hide
  other failures later in the same test.
- **Prefer the specific assertion over a boolean wrapper.** `EXPECT_EQ(a, b)`
  / `EXPECT_NE(a, b)` over `EXPECT_TRUE(a == b)`; `EXPECT_TRUE(cond)` /
  `EXPECT_FALSE(cond)` over `EXPECT_EQ(cond, true)` — the specific form
  prints both operands on failure instead of just `false`.
- **Exception checks use `EXPECT_THROW(expr, Type)`.** Matches the custom
  suite's `CHK_THROWS` exactly; the exception type asserted is unchanged
  from the original test (typically `std::runtime_error`, the common base
  every `JsonPro` exception ultimately derives from).
- **No shared fixture unless a test file actually needs one.** Every test
  ported so far builds its own `Json`/`JsonObject` inline; introduce a
  `TEST_F` fixture only when a category (e.g. a future Lifecycle or
  Concurrency port) genuinely needs shared setup/teardown, not by default.
- **Build/run** via the existing `ctest --test-dir build` entry point — the
  GTest binary is a normal CTest test like any other, not a separate
  workflow.
